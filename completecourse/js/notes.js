	...
	$.widget("que.Notes", {
		...
		updateCaption: function (searchResultCount) {
			if (searchResultCount != undefined) {
				this.element.find(".panel-heading span").text("Notes (" + this.notes.length + " total, " + searchResultCount + " matching)");
			} else {
				this.element.find(".panel-heading span").text("Notes (" + this.notes.length + ")");
			}
		}
	});
});
